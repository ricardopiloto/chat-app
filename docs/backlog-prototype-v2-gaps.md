...
**Actualizado**: 2026-09-06 (049 suspende UI de gravação — G1 volta a backlog; 047 entregou G4)
...
|----|-------------------------|-------------------|---------------------|--------|
| G1 | **Gravar cena…** + diálogo Egress | UI de gravação **suspensa** na 049 (barra do palco removida; sem repor noutro sítio); pipeline/API podem existir | Alta (cunha PRD) | **Backlog futuro** — repor após 049; estava entregue na 006 |
...
| G4 | Canal **privado** (só convidados do canal) | Sem permissões por canal | Média | **Entregue na 047** (público/privado, ACL, papéis, enforce texto/voz) |
...
- Feature **049**: controlos de chamada só no painel; **remove UI de gravação** da mesa → **G1** regressa a backlog futuro.
- Não colocar UI morta para itens ainda Diferidos.
- G1+G2+G5 formam o pacote “gravar ⇄ E2EE” com custódia (G1 UX a repor).
- Canais de voz pré-G5: sem migração de chave — Gravar/Religar desabilitados até recriar (quando G1 voltar).
