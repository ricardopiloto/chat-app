# Feature Specification: Menu de blur da câmera não abre
...
**Depends on**: [015-camera-background-blur](../015-camera-background-blur/) (menu Sem / Leve / Forte na seta da câmera); [020-call-control-icons](../020-call-control-icons/) (chrome Discord do split — possível regressão visual).

## User Scenarios & Testing *(mandatory)*

### User Story 1 - Abrir o menu de fundo pela seta (Priority: P1)

Como participante numa chamada ao vivo, quero clicar na seta junto ao ícone de câmera e ver de imediato as opções de efeito de fundo (Sem blur, Blur leve, Blur forte), para poder escolher ou confirmar o modo sem adivinhar se o clique funcionou.
...
**Independent Test**: Em chamada (incl. modo palco se activo), clicar na seta do split da câmera → menu com as três opções visível e utilizável; `aria-expanded` reflecte aberto.

**Acceptance Scenarios**:

1. **Given** estou numa chamada ao vivo com a barra de controlos visível, **When** clico na seta (chevron) do controlo partido da câmera, **Then** aparece o menu de fundo com as opções **Sem blur**, **Blur leve** e **Blur forte**, legível e não cortado pela barra ou pelo palco.
...
- Câmera desligada: a seta continua a abrir o menu (015); o fix não deve exigir câmera ligada.
- Clique no ícone de câmera (área principal): continua a ligar/desligar câmera **sem** abrir o menu.
...
- **FR-001**: Clicar na seta do split da câmera MUST abrir o menu de opções de fundo da câmera quando este estiver fechado.
...
- **FR-006**: O clique na área principal do ícone de câmera MUST NOT ser necessário para abrir o menu e MUST NOT deixar de alternar a câmera.
...
- **Camera blur menu**: Painel de escolha de fundo (três modos) ancorado à seta do split da câmera.
...
- **SC-005**: Alternar câmera pelo ícone principal continua a funcionar após o fix.
