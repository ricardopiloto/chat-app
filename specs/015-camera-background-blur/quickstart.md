...
Validação manual do blur de fundo. Backend intocado. Precisa de **câmera real** (não «Vídeo de teste») e, para SC-001/006, **duas contas**.
...
1. Entrar na chamada (câmera pode ficar desligada).
2. Clicar na **área principal** Câmera → liga/desliga só a câmera; menu **não** abre.
3. Clicar na **seta** → menu com Sem blur / Blur leve / Blur forte; câmera **não** muda.
4. Com blur leve ou forte, a seta tem **forma** distinta (pip); o ícone da câmera não muda por causa do blur.
5. Escape / clique fora fecha o menu.

## 2. Blur visível para o outro (US1)

1. Conta A: câmera ligada, escolher **Blur leve**.
...
1. Câmera **desligada**. Seta → **Blur forte**.
2. Ligar a câmera pela área principal.
3. **Esperado** (A e B): o primeiro frame visível já está desfocada; nenhum flash do quarto nítido.
4. Recarregar a app, voltar ao canal, ligar câmera (preferência persistida) → mesmo critério.

## 4. Fora de âmbito / indisponível

1. «Vídeo de teste» → sem blur no canvas de teste.
2. (Opcional) Browser sem suporte: escolher leve/forte → mensagem «não disponível»; modo não fica ligado; câmera nítida OK.
